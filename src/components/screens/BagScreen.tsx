...
const GEAR: GearCategory[] = [
  {
    ...
    tip:'護照、機票各備2份實體影本分開存放！旅平險投保證明要英文版，急診時必備。外幣現金建議帶300–500 EUR。',
    items:[
      { id:'passport',   label:'護照',               icon:'🛂' },
      { id:'ticket',     label:'每張機票電子檔',      icon:'✈️' },
      { id:'insurance',  label:'英文版旅平險投保證明', icon:'📄', note:'愛爾蘭急診須出示' },
      { id:'cash',       label:'外幣現金 (EUR)',       icon:'💶', note:'建議帶 300–500 EUR' },
      { id:'creditcard', label:'高回饋信用卡',         icon:'💳', note:'Revolut / 現金回饋卡' },
    ],
  },
  {
    id:'energy', label:'電器區', icon:'⚡', color:'#D4AF37',
    tip:'愛爾蘭插座位置通常很尷尬，延長線是救星。Type G 三孔轉接頭必備，建議備2顆。歐規轉接頭用於荷蘭/法蘭克福轉機。',
    items:[
      { id:'adapter_g',   label:'轉接頭 (英規 Type G)', icon:'🔌', note:'建議備2顆' },
      { id:'adapter_eu',  label:'轉接頭 (歐規)',          icon:'🔌', note:'轉機停留歐洲用' },
      { id:'extcord',     label:'延長線',                  icon:'🔋', note:'當地購買也方便' },
      { id:'powerbank',   label:'行動電源',                icon:'🪫' },
      { id:'charger_c',   label:'Type-C 充電器',           icon:'⚡' },
      { id:'cable_light', label:'Lightning 線',            icon:'📱' },
      { id:'cable_watch', label:'手錶充電線',              icon:'⌚' },
  ...
  },
  {
    ...
    tip:'愛爾蘭的雨是橫著噴的，沒人在撐傘。機能防水外套比雨傘實用十倍。洋蔥穿搭：發熱衣＋毛衣＋羽絨背心＋防水外套靈活應對天氣。',
    items:[
      { id:'jacket',       label:'防水防風保暖外套',   icon:'🧥', note:'最重要單品' },
      { id:'down',         label:'羽絨衣/背心',         icon:'🦺' },
      { id:'heatwear',     label:'發熱衣',              icon:'🧣' },
      { id:'sweater',      label:'毛衣',                icon:'🧶' },
      { id:'longsleeve',   label:'長袖 (3–4件)',        icon:'👕' },
      { id:'tshirt',       label:'短袖 (2–3件)',        icon:'👕' },
      { id:'pants',        label:'褲子 (2件)',          icon:'👖' },
      { id:'underwear',    label:'內衣褲 (多件)',       icon:'👙' },
      { id:'disposable_u', label:'免洗內褲',            icon:'🩲', note:'當地較少見，多帶幾件' },
      { id:'presssocks',   label:'壓力襪',              icon:'🧦', note:'長途飛行必備' },
      { id:'socks',        label:'襪子 (多雙)',         icon:'🧦' },
      { id:'shoes',        label:'鞋子 (防水款)',       icon:'👟', note:'最好能防水' },
      { id:'beanie',       label:'毛帽',                icon:'🎩' },
      { id:'scarf',        label:'圍巾',                icon:'🧣' },
      { id:'sunglasses',   label:'墨鏡',                icon:'🕶️' },
      { id:'eyeglasses',   label:'備用眼鏡',            icon:'👓', note:'建議帶2副，當地很貴' },
      { id:'neckpillow',   label:'頸枕',                icon:'💤', note:'長途飛行必備' },
      { id:'pads',         label:'棉條/衛生棉 (足量)', icon:'🩸', note:'愛爾蘭選擇少且貴', femaleOnly:true },
    ],
  },
  {
    id:'bio', label:'醫藥區', icon:'💊', color:'#E74C3C',
    tip:'台灣藥物便宜且效果好，出發前去診所請醫師開一週常備藥（含處方簽）。愛爾蘭就診費用昂貴，能帶就帶！',
    items:[
      { id:'prescription', label:'診所處方藥 (一週份)',  icon:'💊', note:'感冒/腸胃/過敏/止痛/止瀉' },
      { id:'supplements',  label:'保健食品',              icon:'💊', note:'當地較貴，帶足量' },
      ...
      { id:'contactsol',   label:'隱形眼鏡保養液',        icon:'💧', note:'愛爾蘭賣很貴！多帶幾瓶' },
    ],
  },
  {
    id:'daily', label:'生活用品', icon:'🧴', color:'#6C8EBF',
    tip:'愛爾蘭藥妝比台灣貴很多，保養品從台灣帶足量。洗衣袋和衣物壓縮袋是空間管理神器。吹風機部分宿舍有提供，可先詢問。',
    items:[
      ...
      { id:'laundrybag',  label:'洗衣袋 ×3',     icon:'👜' },
      { id:'thermos',     label:'保溫瓶',        icon:'🍵' },
      { id:'wetwipe',     label:'隨身濕紙巾',    icon:'🧻' },
      { id:'tissue',      label:'隨身衛生紙',    icon:'📄' },
      { id:'ecobag',      label:'帆布包/折疊購物袋', icon:'🛍️' },
      { id:'compressbag', label:'衣物壓縮袋 ×3', icon:'🗜️' },
      ...
      { id:'makeup',      label:'化妝品',        icon:'💄', femaleOnly:true },
      { id:'cottonpad',   label:'化妝棉',        icon:'🌸', femaleOnly:true },
      { id:'lipbalm',     label:'護唇膏',        icon:'💋' },
      { id:'handcream',   label:'護手霜',        icon:'🤲' },
      { id:'toiletbag',   label:'盥洗包',        icon:'🪣' },
      { id:'bodylotion',  label:'身體乳液',      icon:'🧴' },
    ],
  },
  {
    id:'data', label:'數位區', icon:'💻', color:'#7B5E4F',
    tip:'USB 存好履歷、大頭照和文件掃描檔，圖書館或列印店隨時可能用到。手機保護膜出發前先貼好。',
    items:[
      { id:'laptop',    label:'筆電',            icon:'💻' },
      { id:'ipad',      label:'iPad',            icon:'📱', note:'選配，可當電子書/娛樂' },
      { id:'airpods',   label:'AirPods/耳機',    icon:'🎧' },
      { id:'phonefilm', label:'手機保護膜 ×1',   icon:'📱', note:'出發前先貼好' },
      { id:'usb',       label:'USB（履歷/文件）', icon:'💾', note:'存電子文件備份' },
      { id:'cloudbkp',  label:'重要文件雲端備份', icon:'☁️', note:'Google Drive / iCloud' },
    ],
  },
  {
    id:'food', label:'食物補給', icon:'🍜', color:'#B45309',
    tip:'愛爾蘭超市的亞洲食材很有限，調味料（孜然粉/鹹酥雞粉/十三香）、湯包、濾掛咖啡從台灣帶足量。蛋白粉當地相當昂貴。',
    items:[
      { id:'soup',    label:'湯包',            icon:'🍲' },
      { id:'tea',     label:'茶包',            icon:'🍵' },
      { id:'seaweed', label:'海帶芽湯包',      icon:'🌿' },
      { id:'spice',   label:'調味料',          icon:'🧂', note:'孜然粉/鹹酥雞粉/十三香較少見' },
      { id:'coffee',  label:'濾掛咖啡',        icon:'☕', note:'當地常見為即溶咖啡' },
      { id:'snacks',  label:'台灣零食/蛋白粉', icon:'🍡', note:'當地較貴，帶足量' },
    ],
  },
];
